(function () {
    ...
    IframeWatcher.prototype = {
        ...
        hideCTA: function (uid) {
            var self = this,
                stickycta = document.getElementById('ns_facewall__sticky_widget');
            this.onEvent(stickycta, 'click', function (ev) {
                ev.preventDefault();
                stickycta.className = 'ns_facewall__sticky_widget';
                self.sendHideCTAEventToIframe(uid);
            });
        },
        ...
        decideHowToTalkToIframe: function (href) {
            if (window.postMessage) { // if window.postMessage is supported, then support for JSON is assumed
                ...
                this.onEvent(stickycta, 'click', function (ev) {
                    ev.preventDefault();
                    stickycta.className = 'ns_facewall__sticky_widget';
                    self.sendHideCTAEventToIframe(uidForPostMessage);
                });
        ...
        postMessageCallback: function (data) {
            if (this.postBackMessageForThisIframe(data)) {
                ...
                if (this.scrollInTheData()) {
                    if (!this.data.isCTA) {
                        this.enableStickyCTA();
                        this.scrollToAnimated(this.data.scrollPosition, this.data.scrollDuration);

                    } else {
                        if (document.getElementById(this.data.currentFrame)) {
                            if (this.uidForPostMessage.indexOf(this.data.currentFrame.substr(13)) > 0) {
                                window.scrollTo(0, document.getElementById(this.data.currentFrame).offsetTop);
                            }
                        }
                    }
        ...
        scrollInTheData: function (data) {
            return (typeof(this.data.scrollPosition) !== 'undefined');
        },

        stickyHeaderInstruction: function (data) {
            return (typeof(this.data.stickyClassName) !== 'undefined');
        },

        postBackMessageForThisIframe: function (data) {
            return this.data && (data.split('::')[0] === this.uidForPostMessage);
        },
        ...
        enableStickyCTA: function () {
            document.getElementById('ns_facewall__sticky_widget').className += ' ns_facewall__sticky_widget--sticky';
        },
        sendHideCTAEventToIframe: function (uid) {
            var message = {
                    ctahideEvent: 'facewall:hide:cta'
                },
                convictedContainer = document.getElementById('ns_facewall__convicted');

            var pageFrames = document.body.querySelectorAll('iframe');

            for (var pageFrame in pageFrames) {
                var frame = pageFrames[pageFrame],
                    frameParentId;
                if (frame.parentNode) {
                    frameParentId = frame.parentNode.getAttribute('id');
                    if (uid.indexOf(frameParentId.substr(13)) > 0) {
                        document.getElementById(frameParentId).querySelector('iframe').contentWindow.postMessage(uid + '::' + JSON.stringify(message), '*');
                    }
                }
            }
